...
export async function recordFeedback(signal: FeedbackSignal, track: TrackRef): Promise<void> {
  ...
  try {
    if (EXPLICIT.includes(signal)) {
      ...
      const priorExplicit = prior.filter((f) => EXPLICIT.includes(f.signal));
      // Tapping the same opinion again clears it (toggle off).
      const already = priorExplicit.find((f) => f.signal === signal);
      await db.feedback.bulkDelete(priorExplicit.map((f) => f.id));
...
// The current explicit opinion for a track, if any.
export function explicitFor(feedback: Feedback[], sessionId?: string): FeedbackSignal | null {
  if (!sessionId) return null;
  const f = feedback.find((x) => x.sessionId === sessionId && EXPLICIT.includes(x.signal));
  return f ? f.signal : null;
}
...
export function summarize(feedback: Feedback[]): FeedbackSummary {
